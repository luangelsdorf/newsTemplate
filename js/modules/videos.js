...
function renderVid() {
    ...
    if (videos.length !== 0) {
        ...
        videos.forEach(m => {
            var vid = $('<div class="col-lg-4 col-sm-6 my-2" id="video_' + m.id + '"><div class="alert alert-dismissible video-container bg-transparent border-custom shadow p-0 py-2 m-0 flex-center"><figure class="figure m-0 text-center max-w-75"><video class="video-preview figure-img rounded-top max-w-100" src="' + m.conteudo + '" id="figura_vid_' + m.id + '" controls></video><figcaption class="figure-caption text-center text-dark bg-light-2 rounded-bottom py-1"><small id="caption_vid_' + m.id + '"></small><small class="font-weight-bold">' + m.tamanho.toFixed() + 'MB</small></figcaption></figure><button type="button" class="btn close material-icons text-dark" id="deleteVid_' + m.id + '" data-dismiss="alert" aria-label="Close">close</button></div></div>');
            $('#video_container').append(vid);

            document.getElementById("figura_vid_" + m.id).oncanplay = function() {
                videos.forEach(m => {
                    document.getElementById("caption_vid_" + m.id).innerHTML = document.getElementById("figura_vid_" + m.id).videoWidth + " x " + document.getElementById("figura_vid_" + m.id).videoHeight + ", ";
                })
            }
